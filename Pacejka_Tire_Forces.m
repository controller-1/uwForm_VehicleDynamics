...
% IMPORTANT NOTES:
% The Pacejka formula used in this script currently uses steady state values. 
% Future work should look at updating the Pacejka function to include 
% transient slip quantities, see reference [1] pg.185. 
% Several of the quantities supplied or computed here have accompanying 
% simplifying assumptions. Noteworthy assumptions will be noted throughout
% the script.
...
%%%%INSERT COMPUTATION FOR Re
% effective wheel/tire radius due to vertical loading
...
% a_fr = dw_rad - atan((vcog*beta_rad+yawr*r_fr*sin(nu_fr))/...     %% check here
...
% Tire side slip approximation due to camber angle
% kcamb = 0.1;    %coefficient relating 10deg camber to 1deg tire slip angle
% cam_ang = [cam_ang(1,1),cam_ang(1,1),cam_ang(1,2),cam_ang(1,2)];
% adjcam_ang = deg2rad(kcamb*cam_ang);
% for i = 1:length(a_w_nocam)
%     a_w = a_w_nocam + adjcam_ang(1,i);
% end
kcamb = 0.1;    %coefficient relating 10deg camber to 1deg tire slip angle
cam_ang1 = [cam_ang(1,1),-cam_ang(1,1),cam_ang(1,2),-cam_ang(1,2)];
% adjcam_ang = deg2rad(kcamb*cam_ang1);
adjcam_ang = kcamb*cam_ang1;
a_w = zeros(1,4);
for i = 1:length(a_w_nocam)
    %a_w(1,i) = a_w_nocam(1,i) + deg2rad(adjcam_ang(1,i)); % [rad]
...
LSR = zeros(1,4);
for i = 1:4
    %LSR(1,i) = -(vR(1,i)*cos(deg2rad(a_w(1,i))) - uw(1,i))/(uw(1,i)+eps); 
    %LSR(1,i) = -(vR(1,i)*cos(deg2rad(a_w(1,i))) - uw(1,i))/(vR(1,i)*cos(deg2rad(a_w(1,i)))+eps); %Pacejka Definition
